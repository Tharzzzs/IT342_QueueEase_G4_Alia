...
class RegisterStaffViewModel : ViewModel() {

    private val _registerState = MutableLiveData<RegisterState>()
    val registerState: LiveData<RegisterState> = _registerState

    fun registerStaff(firstName: String, lastName: String, email: String, pass: String) {
        ...
        val request = StaffRegisterRequest(firstName, lastName, email, pass)
        ApiClient.apiService.registerStaff(request).enqueue(object : Callback<AuthResponse> {
            override fun onResponse(call: Call<AuthResponse>, response: Response<AuthResponse>) {
                if (response.isSuccessful) {
                    ...
                    if (body?.success == true) {
                        ...
                        _registerState.value = RegisterState.Error(body?.message ?: "Failed to register staff")
                    }
                } else {
